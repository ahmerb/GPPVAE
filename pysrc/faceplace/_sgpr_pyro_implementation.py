...
class SparseGPRegression(GPModel):
    ...
    def __init__(self, X, y, kernel, Xu, noise=None, mean_function=None, approx=None, jitter=1e-6):
        ...
        # Xu is the initial inducing points
        self.Xu = Parameter(Xu) # trainable

        # inits noise to 1 (the noise in y = f + eps) (if not given)
        # X.new_tensor(1) creates new tensor, namely tensor(1.), where it's properties (dtype, device) are same as for X
        noise = self.X.new_tensor(1.) if noise is None else noise
        self.noise = Parameter(noise) # trainable
        self.set_constraint("noise", constraints.positive) # contrained to be +ve
    ...
    def model(self):
        ...
        # solve Luu * W^T = Kuf
        W = Kuf.trtrs(Luu, upper=False)[0].t() # (.t() does transpose of 2d tensor (a matrix))

        # tranforms noise = tensor(sigma^2) into tensor(sigma^2,sigma^2,sigma^2,.....)
        # this is the diagonal of sigma^2 * I (of dimension NxN)
        D = self.noise.expand(N)

        if self.approx == "FITC" or self.approx == "VFE":

            # computes diagonal of Kff = k(X,X)
        ...
        # compute the mean function evaluated at X
        ...
        # or, if y is given, then register these observations as samples from p̃(y|X,Xu)
        else:
            if self.approx == "VFE":
