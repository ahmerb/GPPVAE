...
class DualInputSparseGPRegression(SparseGPRegression):
    ...
    def predict_and_plot(self, Xnew, Wnew, nsamples=3):
        ...
        ax.plot(numpy['Xnew'], numpy['Wnew'], numpy['mu']-2*numpy['Ldiag'], color='#dddddd')  # noqa E226
        ax.plot(numpy['Xnew'], numpy['Wnew'], numpy['mu']+2*numpy['Ldiag'], color='#dddddd')  # noqa E226
        ax.plot(numpy['Xnew'], numpy['Wnew'], numpy['mu'], color='r')
        ax.scatter(numpy['Xu'],
                   np.zeros(numpy['Xu'].shape[-1]) - ax.get_ylim()[0],
                   np.zeros(numpy['Xu'].shape[-1]) - ax.get_zlim()[0], c='r', marker='^')
        ax.scatter(np.zeros(numpy['Xu'].shape[-1]) - ax.get_xlim()[0],
                   numpy['Wu'], np.zeros(numpy['Xu'].shape[-1]) - ax.get_zlim()[0], c='g', marker='^')
...
def testStuff2():
    ...
    N = 100  # num samples
    O = 7    # num inducing points # noqa: E741
    ...
    Y = torch \
        .sin(3 * X).t().mm(torch.cos(2 * W)) \
        .mm(torch.ones(R, N)).t().mm(torch.ones(D, L)) + dists.Normal(0.0, 0.3) \
        .sample(sample_shape=(N, L))
